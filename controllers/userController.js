...
const signUp = async (req, res) => {
  ...
  if (usernameExists) {
    return res
      .status(400)
      .json({ message: 'Username already in use try another one' });
  }
...
const signIn = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(403)
        .json({ message: 'We could not find your account' });
    }
    ...
    return res.status(500).json({
      message: 'Error occurred while signing you in. ' + error.message,
    });
  }
};
...
const updateAvatar = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: 'Could not update user data. ' + error.message });
  }
};
...
const resetPassword = async (req, res) => {
  ...
  try {
      ...
      return res.status(500).json({
        message:
          type === 'password'
            ...
            : type === 'withdrawal' && 'Invalid withdrawal password',
      });
    }
    ...
    return res
      .status(500)
      .json({ message: 'Could not change password ' + error.message });
  }
};
...
const updateData = async (req, res) => {
  ...
  try {
    ...
    const { password, withdrawalPassword, ...userData } =
